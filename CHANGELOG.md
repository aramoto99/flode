...
## [0.60.2] - 2026-09-12 — sample_time ヒント文言を周期語彙に統一

### Changed

- Inspector の sample_time ヒントを整理 (オーナー指摘): 「離散レート」という
  エンジン用語を UI の統一語彙「周期」に置換、冒頭のモード名プレフィックス
  (「継承 (-1):」等) を除去、エラー時の 2 択案内はヒントから外して実行時の
  案内板型エラーメッセージに一本化
  - 継承: 「上流のブロックから周期を継承します。継承できる周期が上流にない
    場合はエラーになります。」
  - 基準クロック: 「シミュレーションの基準周期 dt (現在 {{dt}} s) で動作
    します。dt を変えると追従します。」

## [0.60.1] - 2026-09-12 — sample_time UI の磨き込み

### Changed

- 第 3 モードの呼称: 「明示値」→「**指定**」(「継承」の対語として標準的な語)
- 「指定」時の周期入力を select の**横並びから下のインデント行**に変更
  (「周期 (秒)」ラベル付き。シミュレーション設定の dt_base 明示指定 → 値と
  同じ既存パターン。1 行 1 コントロールの原則に復帰し、狭い Inspector でも
  窮屈にならない)

## [0.60.0] - 2026-09-12 — sample_time の呼称を「継承」に戻し、パレット既定も継承に

### Changed

- GUI の `-1` モードの呼称: 「上流に同期」→「**継承**」(オーナー選好)。
  v0.58.0 で `-1` は継承専用 (継承元がなければエラー) になったため、
  「継承」という語と実際の意味が一致するようになった
- パレットの UnitDelay 既定 sample_time: `"dt"` → **`-1` (継承)** —
  リファレンスツールの既定と同じ。上流にレートがなければ案内板型エラーが
  `"dt"` / 明示値へ導く (fail-closed 既定)
...
- Mux / Demux のパレットアイコンを、キャンバス上の実際の見た目 (プレーンな細い
  縦バー、配線は描かない) に合わせた。実ブロック同様、両者のアイコンは同一になる
  (配線の向きで区別する)

## [0.53.2] - 2026-09-05 — パレットの「SM-B」バッジを削除

### Changed

- パレットの Mux / Demux 等に表示されていた「SM-B」バッジ (内部の信号モデル世代名
  = ベクトルポートを持つブロックの印) を削除。内部用語が UI に漏れており、
  ユーザーに意味が伝わらないため
...
- PARAMETERS 節の行内「削除」「追加」ボタンをテキストからアイコン (× / ＋ の
  小型グリフボタン、hover で tooltip) に変更。`RowActionButton` に
  `icon` variant を追加 (テキストモードも残置)
