...
検証対象:
  1. ``pyflw/__init__.py`` の ``__version__``
  2. ``pyproject.toml`` の ``[project] version``
  3. ``pyflw/web/frontend/package.json`` の ``version``
...
def _read_init_version(repo_root: Path) -> str:
    """``pyflw/__init__.py`` の ``__version__ = "X.Y.Z"`` 行を抽出する。

    末尾コメント (``  # ADR-0029: ...``) は許容し、文字列リテラル部分のみ取り出す。
    """
    init_text = (repo_root / "pyflw" / "__init__.py").read_text(encoding="utf-8")
    for line in init_text.splitlines():
        m = _VERSION_LINE_RE.match(line)
        if m:
            return m.group(1)
    raise RuntimeError("pyflw/__init__.py に __version__ = \"...\" が見つかりません")
...
def _read_package_json_version(repo_root: Path) -> str:
    package_json_path = (
        repo_root / "pyflw" / "web" / "frontend" / "package.json"
    )
...
def main() -> int:
    ...
    versions = {
        "pyflw/__init__.py": _read_init_version(repo_root),
        "pyproject.toml": _read_pyproject_version(repo_root),
        "pyflw/web/frontend/package.json": _read_package_json_version(repo_root),
    }
