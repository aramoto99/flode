...
```bash
# プロジェクトルートで (port 8770 で API を配信)
flode --workspace ./models --no-browser
```
...
```bash
npm run build
# → dist/ に出力後、scripts/deploy-to-server-static.mjs が
#   `flode/server/static/` へ自動コピーする (`.app-version` も更新)
```
