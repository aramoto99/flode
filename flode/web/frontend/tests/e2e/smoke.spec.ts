...
test.describe("flode web GUI smoke (v3.x)", () => {
  test("renders root page with flode branding", async ({ page }) => {
    await page.goto("/");
    // title bar の "flode" + version
    await expect(page.locator("text=flode").first()).toBeVisible({
      timeout: 15_000,
    });
  });
